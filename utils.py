...
def align_and_extract_words(
    ...
    """
    Aligns bounding box files per page with per-page transcription lines,
    grouping boxes by vertical positions to match transcription lines,
    then crops and labels each word accordingly.
    Ensures each filename is unique by appending a counter for duplicates.

    Parameters:
        boxes_folder: Path to folder with sorted box .txt files.
        transcriptions_folder: Path to folder with per-page transcription .txt files.
        images_folder: Path to folder with page images.
        output_folder: Path where cropped word images and labels.csv will be saved.
    """
    boxes_folder = Path(boxes_folder)
    transcriptions_folder = Path(transcriptions_folder)
    images_folder = Path(images_folder)
    output_folder = Path(output_folder)
    output_folder.mkdir(parents=True, exist_ok=True)
    cropped_folder = output_folder / "cropped"
    cropped_folder.mkdir(exist_ok=True)

    metadata = []
    # Track counts of each word for unique filenames
    word_counts = defaultdict(int)

    for box_file in sorted(boxes_folder.glob("*.txt")):
        page_id = box_file.stem.replace("res_", "").replace("_sorted", "")
        image_path = images_folder / f"{page_id}.png"
        trans_path = transcriptions_folder / f"{page_id}.txt"

        if not image_path.exists() or not trans_path.exists():
            continue
        ...
        # Load boxes and compute centroids
        raw_boxes = [l.strip() for l in box_file.open("r", encoding="utf-8") if l.strip()]
        boxes = []
        for raw in raw_boxes:
            parts = re.split(r"[ ,;]+", raw)
            coords = [int(p) for p in parts if p.lstrip('-').isdigit()]
            if len(coords) != 8:
                continue
        ...
        # Load transcription lines
        with open(trans_path, "r", encoding="utf-8") as f:
            trans_lines = [line.strip().split() for line in f if line.strip()]

        # Flatten total words count and sort boxes vertically
        total_words = sum(len(words) for words in trans_lines)
        boxes_sorted = sorted(boxes, key=lambda b: b["y_med"])

        idx = 0
        for words in trans_lines:
            n = len(words)
            group = boxes_sorted[idx: idx + n]
            # Sort horizontally within line
            group_sorted = sorted(group, key=lambda b: b["x_min"])
            for j, b in enumerate(group_sorted):
                word = words[j] if j < len(words) else f"UNK{j}"
                # Determine unique filename
                count = word_counts[word]
                suffix = f"_{count}" if count > 0 else ""
                out_name = f"{word}{suffix}.png"
                word_counts[word] += 1

                # Crop and save
                xs, ys = b["coords"][::2], b["coords"][1::2]
                x0, y0, x1, y1 = min(xs), min(ys), max(xs), max(ys)
                crop = image[y0:y1, x0:x1]
                out_path = cropped_folder / out_name
                cv2.imwrite(str(out_path), crop)
                metadata.append(f"{out_path},{word}")
            idx += n
    ...
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write("\n".join(metadata))
    print(f"Saved {len(metadata)} word crops to {cropped_folder}")
